...
LUA_SHA1 = get_hash(os.path.join(CWD, 'enjin.lua'))
...
class WgetArgs(object):
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'site_id':
                wget_args.extend(['--warc-header', 'enjin-site-id: '+item_value])
                wget_args.append('https://www.enjin.com/page/'+item_value)
            elif item_type == 'profile':
                wget_args.extend(['--warc-header', 'enjin-profile-id: '+item_value])
                wget_args.append('https://www.enjin.com/profile/'+item_value)
            elif item_type == 'thread':
                domain, preset, thread = item_value.split(':', 2)
                wget_args.extend(['--warc-header', 'enjin-{}-thread-id: '.format(domain, thread)])
                wget_args.extend(['--warc-header', 'enjin-{}-thread-id: '.format(preset, thread)])
                wget_args.append('https://{}/m/{}/viewthread/{}'.format(domain, preset, thread))
            elif item_type == 'forum':
                domain, preset, forum = item_value.split(':', 2)
                wget_args.extend(['--warc-header', 'enjin-{}-forum-id: '.format(domain, forum)])
                wget_args.extend(['--warc-header', 'enjin-{}-forum-id: '.format(preset, forum)])
                wget_args.append('https://{}/m/{}/viewforum/{}'.format(domain, preset, forum))
            else:
...
project = Project(
    title='Enjin',
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/thumb/7/77/Enjin-icon.png/600px-Enjin-icon.png" height="50px" title=""/>
        <h2>enjin.com <span class="links"><a href="https://enjin.com/">Website</a> &middot; <a href="http://tracker.archiveteam.org/enjin/">Leaderboard</a></span></h2>
        <p>Archiving Enjin.</p>
    '''
)
